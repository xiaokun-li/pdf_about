...
def search_value_in_pdf(pdf_path, search_value):
    ...
    fh = open(pdf_path, 'rb')
    for page1 in PDFPage.get_pages(fh, caching=True, check_extractable=True):
        print(page1)
        page_interpreter.process_page(page1)
        text1 = fake_file_handle.getvalue()
        start_idx1 = text1.find('334,740.00')
        print(start_idx1)
        print(text1[828:859:])
